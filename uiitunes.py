...
def import_music_folders():
    ...
    for music_folder in music_folders:
        ...
        for file_path in mp3_files:
            track = library_playlist.AddFile(file_path)
            if track is not None:
                found_playlist.AddTrack(track)
                log(f"✅ Added: {os.path.basename(file_path)}")
                added_total += 1
            else:
                log(f"⚠️ Failed to add: {file_path}")

    log(f"\n✅ Import complete. {added_total} tracks added.")
